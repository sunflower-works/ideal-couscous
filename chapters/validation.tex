...
Hardware vertices:
\begin{itemize}
    \item Jetson-class SoC (\gls{cuda}-capable) for accelerated embed/extract + \gls{cnn} inference.
    \item Raspberry Pi 5 (\gls{cpu}-centric) for extraction-side validation + \gls{cnn} inference.
    \item Optional NPU/TPU where available for the \gls{cnn} stage.
\end{itemize}

RTC objective (per-frame budget): keep median latency well below 250\,ms for end-to-end watermark handling on-device.
We report:\injectZWS{rtc}
\begin{itemize}
...
\begin{itemize}
    ...
    \item On \gls{cpu}-only devices, inference follows extraction, preserving the median end-to-end budget.
