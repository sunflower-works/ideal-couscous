...
def main():
    ...
    base_lat = float(lat.get('fp32',{}).get('mean_ms', 'nan')) if 'fp32' in lat else None
    ...
    lines = [
        ...
        'Prec & Mean(ms) & Speedup & dMs & d\% & mAP50 & d mAP50 & mAP50-95 & d mAP50-95 \\', '\\midrule'
    ]
    for r in rows:
        lines.append(
            ' {p} & {mm} & {sp} & {dms} & {dp} & {m50} & {dm50} & {m95} & {dm95} \\'.format(
                ...
                dms=('--' if r['delta_ms_vs_fp32'] is None else fmt(r['delta_ms_vs_fp32'])),
                dp=('--' if r['delta_pct_vs_fp32'] is None else fmt(r['delta_pct_vs_fp32'])),
                ...
                dm95=('--' if r['delta_map50_95'] is None else fmt(r['delta_map50_95'],4)),
            )
        )
    lines += ['\\bottomrule', '\\end{tabular}', f'\\caption{{Precision latency & accuracy deltas (imgsz={args.imgsz}).}}', '\\label{tab:precision_deltas}', '\\end{table}', '']
