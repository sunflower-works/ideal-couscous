...
# ---------------------------------------------------------------------------
NON_NUMERIC_COLS = {
    'algorithm','status','fail_reason','attack_chain','attack_params_json','tags',
    'git_commit','image_id','image_category','attacked_status','attacked_fail_reason',
    'crc_status','blur_bin','source','__source_file','run','group','host_platform'
}
...
def discover_layout() -> str:
    ...
    run_dirs = [p for p in root.iterdir() if p.is_dir() and any((p / g / f'{g}.csv').exists() for g in CFG.required_groups)]
    if run_dirs:
        return 'multi'
        ...
        if csv_path.exists():
            try:
                ...
                if key in groups and not groups[key].empty:
                    groups[key] = pd.concat([groups[key], df], ignore_index=True, sort=False)
                else:
                    groups[key] = df
            except Exception:
                pass
...
ORDERED_COLUMNS: List[str] = [
    'run_uuid','timestamp','git_commit','host_platform','algorithm','round_idx','status','fail_reason',
    'payload_bits_requested','payload_bits_embedded','payload_bits_recovered',
    'ber_percent','ber_bits_wrong','bits_total','success_flag',
    'psnr_db','ssim','embed_ms','extract_ms','total_ms',
    'cpu_pct_mean','cpu_pct_max','rss_max_mib',
    'size_cover_bytes','size_stego_bytes','size_delta_bytes','size_delta_pct',
    'attack_chain','attack_params_json','tags',
    'core_count','openblas_threads','temp_c_max','freq_min_khz',
    'capacity_bits_est','watermark_strength','robustness_score',
    'imperceptibility_norm','imperceptibility_combined','capacity_clamped_row',
    'silent_trunc_flag','crc_status',
    'attacked_status','attacked_fail_reason','attacked_payload_bits_recovered',
    'attacked_ber_percent','attacked_ber_bits_wrong','attacked_bits_total',
    'attacked_success_flag','psnr_attack_db','ssim_attack',
    'attack_delta_psnr','attack_delta_ssim',
    'image_id','image_category','image_blur_metric','image_face_detected'
]

BOOLEAN_DEFAULTS = {'silent_trunc_flag','capacity_clamped_row','success_flag','attacked_success_flag'}
...
def derive_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'imperceptibility_norm' not in wide or wide['imperceptibility_norm'].isna().all():
        if 'psnr_db' in wide:
            wide['imperceptibility_norm'] = (wide['psnr_db'] / 50.0).clip(upper=1.0)
    if 'imperceptibility_combined' not in wide or wide['imperceptibility_combined'].isna().all():
        if {'imperceptibility_norm','ssim'}.issubset(wide.columns):
            wide['imperceptibility_combined'] = 0.5 * (wide['imperceptibility_norm'] + wide['ssim'].fillna(0))
...
def compute_deltas(history: pd.DataFrame) -> pd.DataFrame:
    ...
    for alg, sub in history.groupby('algorithm'):
        ...
        for _, row in sub.iterrows():
            r = row.to_dict()
            if prev is not None:
                for metric in ['embed_ms_mean','extract_ms_mean','ber_mean','success_rate','robustness_mean','cpu_mean','rss_max']:
                    if pd.notna(row.get(metric)) and pd.notna(prev.get(metric)) and prev.get(metric) != 0:
                        r[f'{metric}_delta_pct'] = 100.0 * (row[metric]-prev[metric]) / prev[metric]
            out.append(r)
            prev = row
...
def flag_regressions(deltas: pd.DataFrame) -> pd.DataFrame:
    ...
    def reg(row):
        flags = []
        if pd.notna(row.get('embed_ms_mean_delta_pct')) and row['embed_ms_mean_delta_pct'] > CFG.regress_embed_ms_pct:
            flags.append('embed_ms↑')
        if pd.notna(row.get('extract_ms_mean_delta_pct')) and row['extract_ms_mean_delta_pct'] > CFG.regress_extract_ms_pct:
            flags.append('extract_ms↑')
        if pd.notna(row.get('ber_mean_delta_pct')) and row['ber_mean_delta_pct'] > CFG.regress_ber_abs:
            flags.append('ber↑')
        if pd.notna(row.get('success_rate_delta_pct')) and row['success_rate_delta_pct'] < -CFG.regress_success_pct:
...
def generate_plots(latest_groups: Dict[str, pd.DataFrame]):  # side effects only
    ...
    if not robustness_df.empty and 'attack_chain' in robustness_df.columns:
        ...
        attacks_df = robustness_df.drop_duplicates(subset=['run_uuid','algorithm','attack_chain'])
    # ----------------- Optional CPU/RSS backfill -----------------
    if not baseline_df.empty and {'cpu_pct_mean','rss_max_mib'}.issubset(baseline_df.columns):
        if baseline_df['cpu_pct_mean'].isna().all() or baseline_df['cpu_pct_mean'].eq('').all():
            try:
    ...
    if not baseline_df.empty:
        for metric in ['embed_ms','extract_ms']:
            if metric in baseline_df.columns:
                fig = px.violin(baseline_df, x='algorithm', y=metric, box=True, points='suspectedoutliers', title=f'{metric} distribution')
                fig.show()
    # Throughput
    if not baseline_df.empty and {'payload_bits_embedded','embed_ms'}.issubset(baseline_df.columns):
        tmp = baseline_df.copy()
        tmp['embed_bps'] = (tmp['payload_bits_embedded'] / (tmp['embed_ms']/1000.0)).replace([np.inf,-np.inf], np.nan)
        fig = px.box(tmp, x='algorithm', y='embed_bps', points='outliers', title='Embedding Throughput (bits/s)')
        fig.show()
    # BER distribution
    if not baseline_df.empty and 'ber_percent' in baseline_df.columns:
        fig = px.histogram(baseline_df, x='ber_percent', color='algorithm', nbins=50, title='BER Percent Distribution')
        fig.show()
        if 'robustness_score' in baseline_df.columns:
            fig2 = px.histogram(baseline_df, x='robustness_score', color='algorithm', nbins=50, title='Robustness Score Distribution')
            fig2.show()
    # Strength vs embed time
    if not baseline_df.empty and {'watermark_strength','embed_ms'}.issubset(baseline_df.columns):
        fig = px.scatter(baseline_df, x='watermark_strength', y='embed_ms', color='algorithm', opacity=0.5, trendline='lowess', title='Strength vs Embed Time')
        fig.show()
    # Blur impact
    if not baseline_df.empty and {'image_blur_metric','embed_ms'}.issubset(baseline_df.columns):
        fig = px.scatter(baseline_df, x='image_blur_metric', y='embed_ms', color='algorithm', opacity=0.4, trendline='lowess', title='Blur vs Embed Time')
        fig.show()
        if 'robustness_score' in baseline_df.columns:
            fig2 = px.scatter(baseline_df, x='image_blur_metric', y='robustness_score', color='algorithm', opacity=0.4, trendline='lowess', title='Blur vs Robustness')
            fig2.show()
    # CDF embed/extract baseline vs runtime
    if not baseline_df.empty or not runtime_df.empty:
        cdf_source = pd.concat([
            baseline_df.assign(source='baseline') if not baseline_df.empty else pd.DataFrame(),
            runtime_df.assign(source='runtime') if not runtime_df.empty else pd.DataFrame()
        ], ignore_index=True)
        for metric in ['embed_ms','extract_ms']:
            if metric in cdf_source.columns and not cdf_source.empty:
                tmp = cdf_source[['algorithm','source',metric]].dropna().sort_values(metric)
                if tmp.empty: continue
                tmp['cdf'] = tmp.groupby(['algorithm','source']).cumcount() / (tmp.groupby(['algorithm','source'])[metric].transform('count') - 1)
                fig = px.line(tmp, x=metric, y='cdf', color='algorithm', line_dash='source', title=f'CDF {metric}')
    ...
    if not runtime_df.empty and 'embed_ms' in runtime_df.columns:
        tail = (runtime_df.groupby('algorithm')['embed_ms'].quantile([0.5,0.9,0.95,0.99])
                .unstack().rename(columns={0.5:'p50',0.9:'p90',0.95:'p95',0.99:'p99'}))
        print('Tail latency (embed ms):')
        display(tail)
    # Radar composite (baseline)
    if not baseline_df.empty and 'algorithm' in baseline_df:
        dims = {
            'speed': lambda d: 1.0 / max(d['embed_ms'].mean(), 1e-6) if 'embed_ms' in d else np.nan,
            'robustness': lambda d: d['robustness_score'].mean() if 'robustness_score' in d else np.nan,
            'success': lambda d: d['success_flag'].mean() if 'success_flag' in d else np.nan,
            'strength': lambda d: d['watermark_strength'].mean() if 'watermark_strength' in d else np.nan
        }
        rows = []
        ...
        if not radar_df.empty:
            ...
            for idx, alg in enumerate(algs_sorted):
                ...
                fig.add_trace(go.Scatterpolar(r=vals, theta=categories, fill='toself', name=alg, opacity=opacity))
            fig.update_layout(title='Normalized Composite Radar', polar=dict(radialaxis=dict(range=[0,1])))
            fig.show()
    ...
    if not attacks_df.empty:
        ...
        ber_col = 'ber_percent'
        # Aggregate mean BER
        agg = (atk.dropna(subset=['attack_param'])
        ...
        for atype in ['jpeg','noise','scale','rotate']:
            ...
            fig = px.line(sub.sort_values('attack_param'), x='attack_param', y=ber_col, color='algorithm', markers=True,
        ...
        if len(plotted_types) > 1:
            ...
            facet_fig = px.line(facet_agg.sort_values(['attack_type','attack_param']),
                                x='attack_param', y=ber_col, color='algorithm', markers=True,
                                facet_col='attack_type', facet_col_wrap=2,
...
def parse_args():
    ...
    p.add_argument('--no-plots', action='store_true', help='Skip plot generation even if plotly installed')
    p.add_argument('--just-table', action='store_true', help='Only build unified table; skip history/regressions/plots')
    p.add_argument('--purge-dummy', action='store_true', help='Delete dummy smoke-test CSVs (only if detected as safe).')
    return p.parse_args()
...
def summarize_history(all_runs: Dict[str, Dict[str, pd.DataFrame]]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for run_name, groups in all_runs.items():
        ...
        for g, df in groups.items():
            if df.empty: continue
            (CFG.parquet_dir / f'{run_name}_{g}.parquet').parent.mkdir(parents=True, exist_ok=True)
...
def write_metrics_macros(latest_groups: Dict[str, pd.DataFrame]):
    ...
    if not baseline_df.empty and 'embed_ms' in baseline_df and 'extract_ms' in baseline_df:
    ...
    if not quality_df.empty:
        if 'psnr_db' in quality_df:
            lines.append(f"\\renewcommand{{\\meanPSNR}}{{{_fmt_num(quality_df['psnr_db'].mean(),1)}}}")
        if 'ssim' in quality_df:
            lines.append(f"\\renewcommand{{\\meanSSIM}}{{{_fmt_num(quality_df['ssim'].mean(),3)}}}")
    # Robustness JPEG accuracy
    if not robustness_df.empty:
        df = robustness_df.copy()
        # Determine BER column
        if 'attacked_ber_percent' in df.columns and not df['attacked_ber_percent'].isna().all():
            ber_col = 'attacked_ber_percent'
        else:
            ber_col = 'ber_percent' if 'ber_percent' in df.columns else None
        if ber_col:
            # Extract jpeg quality from attack_chain if present
            if 'attack_chain' in df.columns:
                df['jpeg_quality'] = df['attack_chain'].str.extract(r'jpeg:quality=([0-9]+)').astype(float)
            # Some schemas may have explicit jpeg_quality column
            if 'jpeg_quality' in df.columns:
                acc = 100.0 - df.groupby('jpeg_quality')[ber_col].mean()
                for q in JPEG_QUALITIES_STANDARD:
                    if q in acc.index:
                        val = _fmt_num(acc.loc[q],0)
                    else:
                        val = 'NA'
                    macro_name = {
                        10:'accJPEGten',20:'accJPEGtwenty',30:'accJPEGthirty',40:'accJPEGforty',
                        50:'accJPEGfifty',60:'accJPEGsixty',70:'accJPEGseventy',80:'accJEPEighty',
                        90:'accJPEGninety',100:'accJPEGhundred'
                    }[q]
...
def generate_static_tikz(latest_groups: Dict[str, pd.DataFrame]):
    ...
    # Latency table generation
    baseline_df = latest_groups.get('baseline', pd.DataFrame())
    if not baseline_df.empty and {'embed_ms','extract_ms'}.issubset(baseline_df.columns):
        lat_path = fig_dir / 'latency_table_generated.tex'
        emb_mean = baseline_df['embed_ms'].mean(); emb_sd = baseline_df['embed_ms'].std()
        ext_mean = baseline_df['extract_ms'].mean(); ext_sd = baseline_df['extract_ms'].std()
...
Embed (Jetson) & {emb_mean:.0f}({emb_sd:.0f}) & 250 \\ \hline
Extract (R-Pi 5) & {ext_mean:.0f}({ext_sd:.0f}) & 300 \\ \hline
\end{{tabular}}
\end{{table}}
"""
    ...
    if not robustness_df.empty:
        ...
        if 'jpeg_quality' in df.columns:
            ...
            if ber_col:
                ...
                coords = ' '.join(f'({int(q)},{acc.loc[q]:.0f})' for q in acc.index if not math.isnan(acc.loc[q]))
...
"""
    ...
    # Anchoring cost plot generation placeholder (requires batch_size + fee columns)
...
def write_unified_payload_table(unified: pd.DataFrame):
    ...
    agg['embed_bps_mean'] = np.where(agg['embed_ms_mean']>0, agg['payload_emb_mean'] / (agg['embed_ms_mean']/1000.0), np.nan)
    agg['extract_bps_mean'] = np.where(agg['extract_ms_mean']>0, agg['payload_rec_mean'] / (agg['extract_ms_mean']/1000.0), np.nan)
    def fmt(v, d=0):
    ...
    for _, row in agg.sort_values('algorithm').iterrows():
        ...
        succ = fmt(row['success_rate']*100,1) if not pd.isna(row['success_rate']) else 'NA'
        ...
        lines.append(f"{alg} & {req} & {emb} & {rec} & {ber} & {succ} & {ebps} & {xbps} & {psnr} & {ssim} & {ems}/{xms} \\\\")
